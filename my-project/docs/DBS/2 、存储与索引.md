...
| 优点 | 空间利用率高，没有空洞（fragmentation）                      | **不移动记录的位置**，而是使用 **bitmap 或 slot 标记记录是否存在**。 |
...
| 填充（Padding）❌                                             | 分隔符（Delimiter / CSV）❌                                   | 头指针 / 偏移表（Record Header）✅                            |
| ------------------------------------------------------------ | ------------------------------------------------------------ | ------------------------------------------------------------ |
| 把 VARCHAR 固定长度。给出冗余空间保证变量可以被存储。缺点就是空间利用率低下 | 使用逗号分割变量。问题一是如果文本里有逗号会混乱；二是找第 i 个字段需要扫描 | ![record-header.svg](../assets/images/DBS/record-header.svg)Record Header 记录每个字段的 offset以及 NULL bitmap。 |
