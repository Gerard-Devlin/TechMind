...
动态规划（Dynamic Programming，DP）把一个大问题拆成若干相互重叠的子问题，并保存已经计算过的结果，避免重复计算。
...
2. **转移**：当前状态如何由更小的状态得到。
3. **初始化**：最小规模的问题答案是什么。
4. **遍历顺序**：计算当前状态时，它依赖的状态是否已经得到。

!!! tip "什么时候考虑动态规划？"
    如果问题具有**最优子结构**，并且不同决策会反复遇到相同子问题，就可以尝试定义状态并写出状态转移方程。

---

## 二、$0/1$ 背包问题

有 $n$ 个物品，第 $i$ 个物品的体积为 $w_i$、价值为 $v_i$。每个物品最多选择一次，求容量为 $W$ 的背包能够装下的最大价值。
...
如果压缩成一维数组，容量必须从大到小遍历，防止同一个物品在一轮中被重复使用：
...
完全背包允许每个物品被选择任意次。`dp[i][j]` 仍表示只考虑前 $i$ 个物品、容量为 $j$ 时的最大价值。

- 不选：`dp[i][j] = dp[i - 1][j]`
- 选择：`dp[i][j] = dp[i][j - w[i]] + v[i]`

选择第 $i$ 个物品后仍然可以继续选择它，因此第二项来自当前行，而不是上一行：
...
压缩成一维数组后，容量要从小到大遍历，让本轮刚更新的 `dp[j - w[i]]` 可以继续参与转移：
...
!!! warning "遍历方向"
    `0/1` 背包从大到小枚举容量；完全背包从小到大枚举容量。两者的区别来自一个物品能否被重复使用。

---

## 四、子序列 DP

以最长上升子序列为例，`dp[i]` 表示**以第 $i$ 个元素结尾**的最长上升子序列长度，初始值为 $1$。

对于所有 $j<i$，如果 $a_j<a_i$，就可以把 $a_i$ 接到以 $a_j$ 结尾的序列后面：

$$
dp[i] = \max\left(dp[i],\ dp[j]+1\right)
$$

枚举所有 $(j,i)$ 的时间复杂度为 $O(n^2)$，最终答案是 $\max_i dp[i]$。
...
??? tip
	`dp[i][j]` 表示从顶点走到第 $i$ 行第 $j$ 个位置时，能够得到的最大路径和。
...
??? tip
	`dp[i]` 表示以第 $i$ 项结尾的最长上升子序列长度。
	
	> 当 $j<i$ 且 $a_j<a_i$ 时：$dp[i] = \max(dp[i], dp[j] + 1)$。
...
??? tip
	...
	$$
	s_i>s_j \Rightarrow dp_{i,j}=\text{true}
	$$
	$$
	s_i=s_j \Rightarrow dp_{i,j}=dp_{i+1,j-1}
	$$
	$$
	s_i<s_j \Rightarrow dp_{i,j}=\text{false}
	$$
...
??? tip
    ...
    ① 不选第 `i` 个数：`dp[i][j][k] = dp[i - 1][j][k]`
    
    ② 选择第 `i` 个数：`dp[i][j][k] += dp[i - 1][j - 1][k - i]`
