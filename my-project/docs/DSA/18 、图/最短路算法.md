...
对所有边遍历n次，松弛所有边至无法继续松弛，从而获得单源最短路。如果遍历n次后发现仍然可以进行松弛操作，则说明图一定存在负环。时间复杂度为 $O(nm)$ 。

```c++
vec<array<int, 3>> edge(m);
vec<int> dis(n + 1, inf);
dis[1] = 0;
bool flag = 0; //如果最终结果为1则代表存在负环
for (int i = 1; i <= n; i++) {
    flag = 0;
    for (auto &[u, v, w] : edge) {
        int ndis = dis[u] + w;
...
然而，遍历所有边的操作通常是低效的。注意到只有当一个节点 $v$ 被修改过后，才能够有可能优化以 $v$ 为起点的路径，因此使用队列，将所有 $v$ 入队，并通过 $cnt$ 数组判断最短路经过的边数，从而判断负环。队列优化过后的算法名为SPFA,即 Shortest Path Faster Algorithm。
...
使用了SPFA算法和 $Dijkstra$ 算法得到全源最短路，通过建立虚拟源点，并对虚拟源点跑SPFA，获得一个类似于**点的势能**的概念的 $h$ 数组，并通过该数组对所有边进行特殊处理保证其非负，解决了 $Dijkstra$ 算法无法应用于负权边的问题。时间复杂度 $O(nmlog(m))$ 。

```c++
// 原题网址https://www.starrycoding.com/problem/100
#include <bits/stdc++.h>
using namespace std;
...
void dijkstra(int st, ll dis[]) {
    ...
    while (!pq.empty()) {
        ...
        if (vis[x])
            continue;
        vis[x] = 1;
        for (const auto &[w, nx] : g[x]) {
            if (!vis[nx] && dis[nx] > dis[x] + w) {
                dis[nx] = dis[x] + w;
                pq.push({dis[nx], nx});
            }
        }
