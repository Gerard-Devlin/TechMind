...
如果一棵二叉搜索树的高度为 $h$，最坏情况下查找一个关键字需要比较 $h$ 次，因此查找时间复杂度为 $O(h)$。当树接近平衡时，$h=O(\log n)$，插入、删除和查找都可以在 $O(\log n)$ 时间内完成，其中 $n$ 是结点总数。

但 $O(\log n)$ 只在树形较平衡时成立。最坏情况下，二叉搜索树会退化为链表，例如每个结点都只有右孩子，此时增、删、改、查的时间复杂度都会退化为 $O(n)$。
...
有一棵包含 $n\ (n\leq10^6)$ 个结点的二叉树，根结点编号为 $1$。给出每个结点的两个子结点编号；如果某个方向没有子结点，则对应编号为 `0`。

建树后，依次输出它的前序、中序和后序遍历结果。
